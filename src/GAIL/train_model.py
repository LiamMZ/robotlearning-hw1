...
def train_discrim(discrim, memory, reward_memory, discrim_optim, demonstrations, args):
    memory = np.array(memory)
    reward_memory = np.array(reward_memory)
    states = np.vstack(memory[:, :40])
    actions = list(memory[:, 40:])
    rewards = list(reward_memory[:, 0])
    masks = list(reward_memory[:, 1])
    vals = torch.Tensor(memory)
    criterion = torch.nn.BCELoss()

    for _ in range(args.discrim_update_num):
        # learner = discrim(torch.cat([states, actions], dim=1))
        learner = discrim(vals)
    ...
    learner_acc = ((discrim(vals) > 0.5).float()).mean()

    return expert_acc, learner_acc


def train_actor_critic(actor, critic, memory, reward_memory, actor_optim, critic_optim, args):
    memory = np.array(memory)
    reward_memory = np.array(reward_memory)
    states = np.vstack(memory[:, :40])
    actions = list(memory[:, 40:])
